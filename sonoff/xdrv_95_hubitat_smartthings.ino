/*
  xdrv_02_hubitat_smartthings.ino - Hubitat / SmartThings support for Sonoff-Tasmota
...
boolean HubitatCommand(void)
{
  ...
  if (-1 == command_code) {
    ...
    if ((data_len > 0) && (data_len < sizeof(Settings.hubitat_host))) {
      strlcpy(Settings.hubitat_host, (SC_CLEAR == Shortcut(dataBuf)) ? "" : (SC_DEFAULT == Shortcut(dataBuf)) ? HUBITAT_HOST : dataBuf, sizeof(Settings.hubitat_host));
      //restart_flag = 2;
    }
    ...
    if (payload16 > 0) {
      Settings.hubitat_port = (1 == payload16) ? HUBITAT_PORT : payload16;
      //restart_flag = 2;
    }
